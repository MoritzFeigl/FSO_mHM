"""This file is shamelessly copied from xarray 0.15 (core.combine.py)"""
...
def get_index_from_ranges(ranges, ref_ranges, ascending=True):
    def _get_index(_range, ref_ranges, comparison):
        for index, ref_range in enumerate(ref_ranges):
            if comparison(_range, ref_range):
                return index
        # this should not happen
        return -1

    # set the boundary comparison depending on the sort order
    if ascending:
        def comparison(_range, ref_range):
            return _range[0] >= ref_range[0] and _range[1] <= ref_range[1]
    else:
        def comparison(_range, ref_range):
            return _range[0] <= ref_range[0] and _range[1] >= ref_range[1]

    return [_get_index(_range, ref_ranges, comparison) for _range in ranges]


def _union_non_overlapping_ranges_int_asc(begin, end, unions):
...
def _union_non_overlapping_ranges_int_desc(begin, end, unions):
    if unions and unions[-1][1] <= begin + 1:
        unions[-1][1] = min(unions[-1][1], end)
    else:
        unions.append([begin, end])
    return unions


def _union_non_overlapping_ranges_float_asc(begin, end, unions):
    if unions and unions[-1][1] > begin:
        unions[-1][1] = max(unions[-1][1], end)
    else:
        unions.append([begin, end])
    return unions
...
def _union_non_overlapping_ranges_float_desc(begin, end, unions):
    if unions and unions[-1][1] < begin:
        unions[-1][1] = min(unions[-1][1], end)
    else:
        unions.append([begin, end])
...
def union_non_overlapping_ranges(ranges, ascending=True):
    # condition: all ranges are sorted according to ascending parameter
    # adapted from https://stackoverflow.com/a/15273749/2342319
    unions = []
    flag_all_int = all(isinstance(values, int) for items in ranges for values in items)
    comparison = {
        (True, True): _union_non_overlapping_ranges_int_asc,
        (False, True): _union_non_overlapping_ranges_float_asc,
        (True, False): _union_non_overlapping_ranges_int_desc,
        (False, False): _union_non_overlapping_ranges_float_desc,
    }[(flag_all_int, ascending)]

    for begin, end in sorted(ranges, reverse=not ascending):
        unions = comparison(begin, end, unions)
    ...
    for dim in ds0.dims:
        ...
        if dim in ds0:
            ...
            if not all(index.equals(indexes[0]) for index in indexes[1:]):
                ...
                ranges = [(index[0], index[-1]) for index in indexes]
                unions = union_non_overlapping_ranges(ranges, ascending=ascending)
                order = get_index_from_ranges(ranges, unions, ascending=ascending)
