...
for (run_to_validate_name in runs){
  ...
  basin_info <- read.table("/home/lv71468/mfeigl/FSO_mHM/2020_fso_mhm/scripts/10_validation/LUT_german_basins_with_properties_and_water_balance.txt", 
                           sep = ";", header = TRUE)
  zink_basin <- read.csv2("/home/lv71468/mfeigl/FSO_mHM/2020_fso_mhm/scripts/10_validation/zink_used_basins.csv")
  basin_info <- basin_info[basin_info$Stat_ID %in% zink_basin$Station_ID, ]
  ...
  for(basin in basins){
    ...
    writeLines(basin_script, paste0("mhm_run_scripts/run_basin_", count, ".sh"))
  }
  # adapt mhm paths
  ...
  # run mhm from tf for small basins
  ...
  basin_folders <- all_basin_folders[sapply(basins, function(x) grep(x, all_basin_folders))]
  ...
  set_start <- 1
  bash_file <- readLines("/home/lv71468/mfeigl/FSO_mHM/2020_fso_mhm/scripts/10_validation/02_small_basin_validation_run_template.sh")
  setwd(paste0(run_folder, "/mhm_run_scripts"))
  ...
  for(set in 1:basin_sets){
    ...
    for(set_id in sets[-steps]){
      bash_cmd <- c(bash_cmd, 
                    paste0("bash ", run_folder, "/mhm_run_scripts/run_basin_",set_id, ".sh &> ",
                           run_folder, "/mhm_run_scripts/run_basin_", set_id, ".log & ")
      )
    }
    bash_cmd <- c(bash_cmd, 
                  paste0("process_id=$! bash ", run_folder, "/mhm_run_scripts/run_basin_", 
                         sets[steps], ".sh &> ", run_folder, 
                         "/mhm_run_scripts/run_basin_", sets[steps], ".log & wait $process_id"))
    
    set_bash_file <- gsub("#SBATCH -J val", paste0("#SBATCH -J val", set), bash_file)
    set_bash_file <- gsub("#SBATCH --output=val.out" , paste0("#SBATCH --output=val", set, ".out"), set_bash_file)
    set_bash_file[length(set_bash_file) + 1] <- paste0(bash_cmd, collapse = "")
    writeLines(set_bash_file, paste0(run_folder, "/mhm_run_scripts/run_script_", set, ".sh"))
    system(paste0("chmod 775 ", run_folder, "/mhm_run_scripts/run_script_", set, ".sh"))
    system(paste0("sbatch run_script_", set, ".sh"))
    set_start <- set_start + steps
    cat("Started computing basins:", paste0(sets, collpase = ", "), "\n")
  }
  # run the last set
  set <- basin_sets + 1
  sets <- set_start:(set_start + (rest_basins-1))
  bash_cmd <- NULL
  for(set_id in sets[-rest_basins]){
    bash_cmd <- c(bash_cmd, 
                  paste0("bash ", run_folder, "/mhm_run_scripts/run_basin_",set_id, ".sh &> ",
                         run_folder, "/mhm_run_scripts/run_basin_", set_id, ".log & ")
    )
  ...
  bash_cmd <- c(bash_cmd, 
                paste0("process_id=$! bash ", run_folder, "/mhm_run_scripts/run_basin_", 
                       sets[rest_basins], ".sh &> ", run_folder, 
                       "/mhm_run_scripts/run_basin_", sets[rest_basins], ".log & wait $process_id"))
  set_bash_file <- gsub("#SBATCH -J val", paste0("#SBATCH -J val", set), bash_file)
  set_bash_file <- gsub("#SBATCH --output=val.out" , paste0("#SBATCH --output=val", set, ".out"), bash_file)
  ...
  running_jobs <- system("squeue -u mfeigl", intern = TRUE)
  while(any(grepl("val[0-9]", running_jobs))){
    Sys.sleep(5)
    running_jobs <- system("squeue -u mfeigl", intern = TRUE)
  }
  ...
  summary(basin_results)
  write.csv(basin_results, paste0(run_to_validate, "/all_validation_basins_results.csv"),
            row.names = FALSE)
  
  # get basin Q predictions
  q_predictions <- NULL
  for(basin in basin_folders){
    
    cat(which(basin_folders == basin), "/", length(basin_folders), "\r")
    try({
      output <- read.table(
        paste0("/home/lv71468/mfeigl/FSO_mHM/mHM_setup/FSO_mHM_all_basins/output", 
               "/", basin, "/daily_discharge.out"), header = TRUE)
      output[output == -9999] <- NA
      output$date <- as.POSIXct(paste0(output$Year, "-", output$Mon, "-", output$Day))
      if(is.null(q_predictions)) {
        q_predictions <- data.frame("date" = output$date)
      }
      q_predictions <- merge(q_predictions, output[, 5:7], by = "date", all.x = TRUE)
    })
  }
  write.csv(q_predictions, paste0(run_to_validate, "/small_basins_predictions.csv"),
            row.names = FALSE)
