#!/usr/bin/env python
# encoding: utf-8

"""
File Name   : sample_small_basins
Project Name: 2020_FSO_mHM
Description : insert your description here, if applicable
Author      : ottor
Created     : 10.11.20 13:06
"""

# IMPORTS
import pandas as pd
from random import choice
import os
import json
os.chdir("C:/Users/morit/Dropbox/Diss/FSO_mHM/2020_fso_mhm/scripts/10_validation/")
# GLOBAL VARIABLES
BASIN_PROPERTY_FILE = 'validation_basins_info.txt' # 'LUT_german_basins_with_properties_and_water_balance.txt'
AREA_CUTOFF = 7500


# FUNCTIONS
def choose(arg):
    if len(arg) == 0:
        return None
    else:
        return choice(arg)


# CLASSES

# SCRIPT
if __name__ == '__main__':
    ds = pd.read_csv(BASIN_PROPERTY_FILE, sep=';')
    updated_ds = ds[ds['catArea'] < AREA_CUTOFF].\
        assign(pet_div_p=ds['PET[mm_a-1]'] / ds['Precip[mm_a-1]'],
               et_div_p=ds['AET[mm_a-1]'] / ds['Precip[mm_a-1]'])
    updated_ds.plot.scatter(y='et_div_p', x='pet_div_p')
    budyko_bins = pd.cut(updated_ds['pet_div_p'], bins=21)
    selection = [choose(updated_ds[budyko_bins == cat]['Stat_ID'].values) for cat in budyko_bins.cat.categories]
    chosen_basins = [str(_) for _ in selection if _ is not None]
    with open('sampled_validation_basins.txt', 'w') as f:
        f.write(json.dumps(chosen_basins))
    pd.DataFrame({"sampled_validation_basins": chosen_basins}).to_csv("sampled_validation_basins.csv", index=False)
    print(chosen_basins)